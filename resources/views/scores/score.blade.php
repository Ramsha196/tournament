...
<div class="container">
    <div class="row">
        <div class="col-md-6">
            ...
            <form action="{{ route('createScore', $tournament->id) }}" method="post">

                <input name="_token" type="hidden" value="{{ csrf_token() }}"/>
                <div class="form-group">
                    <label for="language" >No of Scores</label>
                    <br>
                    <input type="number" name="no_of_scores">
                </div>
                <br>
                    ...
                    <select name="player_id" id="">
                        <option disabled selected value>Choose Player</option>
                          @foreach($player as $players)
                                <option value="{{ $players->id }}">{{ $players->player_name }}</option>
                          @endforeach
                    </select>
